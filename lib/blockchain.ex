defmodule Blockchain do
  ...
  def txs_filter(txs, type) do: Enum.filter(txs, fn(t) -> t.data.__struct__ == type end) 
  def being_spent(txs) do: txs |> txs_filter(:Elixir.SpendTx) |> Enum.map(fn(t) -> t.data.amount end) |> Enum.reduce(0, &(&1+&2))
  def prev_block(block) do: KV.get(block.data.hash)
  ...
  def get_helper(h) do: KV.get(to_string(h))
  def get_block(h) do
