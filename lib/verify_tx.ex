defmodule VerifyTx do
  ...
  def check_logic(tx, txs, prev_hash) do
    f = [
      {:Elixir.SignTx, &(SignTransaction.check(&1, &2, prev_hash))},
      {:Elixir.SpendTx,               &(Spend.check(&1, &2))},
      {:Elixir.Spend2WaitTx,     &(Spend2Wait.check(&1, &2))},
      {:Elixir.Wait2BondTx,       &(Wait2Bond.check(&1, &2))},
      {:Elixir.Bond2SpendTx,     &(Bond2Spend.check(&1, &2))},
      {:Elixir.SlasherTx,           &(Slasher.check(&1, &2))},
      {:Elixir.RevealTx,             &(Reveal.check(&1, &2))},
      {:Elixir.ToChannelTx,       &(ToChannel.check(&1, &2))},
      {:Elixir.ChannelBlockTx, &(ChannelBlock.check(&1, &2))},
      {:Elixir.CloseChannelTx, &(CloseChannel.check(&1, &2))},
      {:Elixir.OracleTx,             &(Oracle.check(&1, &2))},
      {:Elixir.JudgementTx,       &(Judgement.check(&1, &2))},
      {:Elixir.WinTx,                   &(Win.check(&1, &2))},
    ]
