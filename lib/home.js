...
refresh_my_balance();
window.setInterval(refresh_my_balance, 10000);

channel = {};
function refresh_channel() {
    var x = local_get("channel_get&".concat(server_status.pubkey));
    refresh_helper(x, function(){ channel = xml_out(x);});
};
refresh_channel();
...
// message and refresh messages needs to be re-thought. we should store a large data-structure of all the messages, and continually refresh it.
...
function refresh_messages() {
    empty_messages();
    pub = document.getElementById("other").value;
    console.log("peer pub ".concat(pub));
...
function server() {return {"ip":"45.55.5.85","port":PORT}};
