    ...
    <font>partner pubkey: </font><textarea id="other" rows="1" cols="80">BF3qa63/nk20IOi2mYxwxdQaaCgteOLoxOWjXDApRdUOfDvoCvCmwV6vNMDaC39tkhReV25BlPvE+JSMr6p/I5g=</textarea>
...
function refresh_inbox_size() {
    ...
    // console.log("inbox size ".concat(inbox_size));
};
refresh_inbox_size();
...
document.getElementById("switch_partner").onclick=function() {
    ...
    peer_id = peer_id + 1;
    if (ps.length > 0) {
	document.getElementById("other").value = ps[peer_id % inbox_peers.length];
    };
};
