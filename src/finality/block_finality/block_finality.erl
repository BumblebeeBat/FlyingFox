...
init(ok) -> 
    H = top(),
    Genesis = sign:empty(block_tree:empty_block()),
    if
	H == 0 -> append_helper(term_to_binary(Genesis));%store the genesis block into the database.
	true -> 0 = 0
    end,
    {ok, []}.
...
append(Block) -> gen_server:cast(?MODULE, {append, term_to_binary(Block)}).
