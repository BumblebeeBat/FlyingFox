...
repo(Acc, Target, Fee, Channels) ->
    ...
    #repo{acc = Acc, nonce = Nonce + 1, target = Target, fee = Fee, channels = Channels}.
low_balance(Acc, TotalCoins, NewHeight) -> accounts:min_balance(Acc, TotalCoins) div 2 > accounts:new_balance(Acc, TotalCoins, NewHeight).
