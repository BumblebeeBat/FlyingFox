...
test() -> 
    ...
    tx_pool_feeder:absorb(keys:sign(spend_tx:spend(1, 1, 0))),
    tx_pool_feeder:absorb(keys:sign(spend_tx:spend(2, 1, 0))),
    ...
    SignedChannelHashlock22 = sign_tx(channel_timeout_tx:timeout_channel(2, sign:set_revealed(SignedChannelHashlock12, [[hash:doit(1)]])), Pub2, Priv2, 2),
    %Two problems here. 
...
long_test() -> 
    ...
    F2 = fun() -> tx_pool_feeder:absorb(keys:sign(sign_tx:sign())), reveal:reveal(), buy_block() , timer:sleep(200) end,
