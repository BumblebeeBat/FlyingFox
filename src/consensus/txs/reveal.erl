...
doit(Tx, ParentKey, Channels, Accounts, TotalCoins, Secrets, NewHeight) ->    
    ...
    io:fwrite("reveal number "),
    io:fwrite(integer_to_list(Number)),
    io:fwrite("\n"),
    io:fwrite("secret Hash "),
    io:fwrite(base64:encode(SH)),
    io:fwrite("\n"),
    io:fwrite("parentKey "),
    io:fwrite(base64:encode(ParentKey)),
    io:fwrite("\n"),
    %Secret = block_tree:secret(Number+1, SH, ParentKey, Secrets),
    %Secret = true,
