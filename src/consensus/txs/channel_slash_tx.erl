...
doit(Tx, ParentKey, Channels, Accounts, TotalCoins, Secrets, NewHeight) ->
    ...
    SignedSlashedCB = sign:empty(SlashedCB),
    NewReveal = channel_block_tx:reveal_union(SlashedCB, sign:revealed(SignedCB), OriginTx, sign:revealed(SignedOriginTx)),
    NewSignedCB = sign:set_revealed(SignedSlashedCB, NewReveal),
    SlasherType1 = (channel_block_tx:nonce(CB) > channel_block_tx:nonce(OriginTx)),
    SlasherType2 = not(sign:revealed(SignedCB) == NewReveal),
