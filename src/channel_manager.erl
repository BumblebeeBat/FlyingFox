...
-export([start_link/0,code_change/3,handle_call/3,handle_cast/2,handle_info/2,init/1,terminate/2, unlock_hash/2,hashlock/3,spend/2,recieve/3,read/1]).
...
new_store(ChId, Ch) -> 
    F = #f{channel = Ch, unlock = repeat(length(channel_block_tx:channel_bets(Ch)), 28)},
    store(ChId, F).
...
recieve(ChId, MinAmount, NewCh) ->
    F = read(ChId),
    Ch = F#f.channel,
    channel_block_tx:channel_block_amount(NewCh),
    0.
